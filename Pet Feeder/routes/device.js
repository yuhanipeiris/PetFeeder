...
Router.put("/", urlencodedParser, (req, res) => {
    ...
    mysqlConnection.query("SELECT * FROM `device` WHERE `deviceId`='" + deviceId + "' AND `password`='" + password + "' ", (err, rows, fields) => {
        ...
        if (rows[0].id != undefined) {
            mysqlConnection.query("UPDATE `user_device` SET `slot1Status` = '" + slot1 + "',`slot2Status` = '" + slot2 + "',`slot3Status` = '" + slot3 + "' WHERE `user_device`.`device` = " + rows[0].deviceId + ";", (err, rows, fields) => {
                if (!err) {
                    response = {
                        success: true,
                    };
